     SELECT
          (select  lot.mvt_dt_validade
                  ...
                  and NF.NOT_IN_NUMERO = T.NOT_IN_NUMERO  -- numero da nota
     ...
     FROM (SELECT DISTINCT ORG_TAB_IN_CODIGO,
            ...
            FROM (SELECT A.ORG_TAB_IN_CODIGO,
                         ...
                         L.MED_TAB_IN_CODIGO,
                         L.MED_PAD_IN_CODIGO,
                         L.MED_IN_SEQUENCIA + 500 AS MED_IN_SEQUENCIA500,
                         L.MED_IN_SEQUENCIA,
                         L.ENS_RE_QTTOLERINF,
                         L.ENS_RE_QTTOLERSUP,
                         L.ENS_RE_INSPECAO,
                   ...
                   WHERE A.ORG_TAB_IN_CODIGO      = B.ORG_TAB_IN_CODIGO
                     ...
                     AND K.ENS_IN_ENSAIO    = L.ENS_IN_ENSAIO
                                          
                     UNION ALL
                     
                     --SELECT LAUDO NIVEL 2
                   ...
                   WHERE A.ORG_TAB_IN_CODIGO      = B.ORG_TAB_IN_CODIGO
                     ...
                     --INCLUSÃO SELECT NIVEL 2
                     AND E.PRO_TAB_IN_CODIGO = M.PRO_TAB_IN_CODIGO
                     AND E.PRO_PAD_IN_CODIGO = M.PRO_PAD_IN_CODIGO
                     AND E.PRO_IN_CODIGO     = M.PRO_IN_CODIGO

                     --AND UPPER(C.MVL_ST_LOTEFORNE) = UPPER(F.LIN_ST_LOTEFORNE) 
                     --ALTERAÇÃO REALIZADA PARA RETIRAR POSSÍVEIS ESPAÇOS EM BRANCO (ERRO DE DIGITAÇÃO)
                     AND UPPER(REPLACE(C.MVL_ST_LOTEFORNE,' ')) = UPPER(REPLACE(M.MVL_ST_LOTEFORNED,' '))
                     
                     AND F.PRO_TAB_IN_CODIGO = M.PRO_TAB_SANIVEL2
                     AND F.PRO_PAD_IN_CODIGO = M.PRO_PAD_SANIVEL2
                     AND F.PRO_IN_CODIGO     = M.PRO_IN_SANIVEL2
                   
                     --AND UPPER(C.MVL_ST_LOTEFORNE) = UPPER(F.LIN_ST_LOTEFORNE) 
                     --ALTERAÇÃO REALIZADA PARA RETIRAR POSSÍVEIS ESPAÇOS EM BRANCO (ERRO DE DIGITAÇÃO)
                     AND UPPER(REPLACE(F.LIN_ST_LOTEFORNE,' ')) = UPPER(REPLACE(M.PRO_ST_LOTEFORNENIVEL2,' '))
                   ...
                   WHERE A.ORG_TAB_IN_CODIGO      = B.ORG_TAB_IN_CODIGO
                     ...
                     AND F.PRO_TAB_IN_CODIGO = M.PRO_TAB_SANIVEL3
                     AND F.PRO_PAD_IN_CODIGO = M.PRO_PAD_SANIVEL3
                     AND F.PRO_IN_CODIGO     = M.PRO_IN_SANIVEL3
                   
                     --AND UPPER(C.MVL_ST_LOTEFORNE) = UPPER(F.LIN_ST_LOTEFORNE) 
                     --ALTERAÇÃO REALIZADA PARA RETIRAR POSSÍVEIS ESPAÇOS EM BRANCO (ERRO DE DIGITAÇÃO)
                     AND UPPER(REPLACE(F.LIN_ST_LOTEFORNE,' ')) = UPPER(REPLACE(M.PRO_ST_LOTEFORNENIVEL3,' '))
